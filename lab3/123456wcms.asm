...
; Вызов функции wsprintf для форматирования строки:
; записывает в buf строковое представление числа msg согласно ifmt.

invoke WriteConsoleA, stdout, ADDR buf, BSIZE, ADDR cWritten, 0
; Использует функцию WriteConsoleA для записи содержимого буфера buf в консоль,
; где stdout указывает на стандартный вывод, BSIZE задает максимальное количество 
; символов, а cWritten будет содержать количество записанных символов.

; 1 // Дескриптор консольного ввода/вывода HANDLE 
; 2 // Буфер, содержащий данные для вывода/записи 
; 3 // Количество символов для вывода/ввода DWORD
; 4 // Количество записанных/прочитанных символов
; 5 // 0 - не используется
; addr передаёт адрес

invoke ExitProcess, 0
; Завершает выполнение программы, передавая 0 как код завершения.

end start
; Указывает конец программы и определяет метку start как точку входа для выполнения.